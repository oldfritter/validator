...
func RegisterDefaultTranslations(v *validator.Validate, trans ut.Translator) (err error) {
	...
	translations := []struct {
		...
		},
		{
			...
			translation: "{0} doit être un numéro de téléphone fixe chinois valide",
			override:    false,
		},
		{
			tag:         "chinese_mobilephone",
			translation: "{0} doit être un numéro de téléphone mobile china valide",
			override:    false,
		},
		{
			tag:         "url",
		...
		{
			tag:         "notin",
			translation: "{0} ne doit pas être l'un des [{1}]",
			override:    false,
			customTransFunc: func(ut ut.Translator, fe validator.FieldError) string {
				s, err := ut.T(fe.Tag(), fe.Field(), fe.Param())
				if err != nil {
