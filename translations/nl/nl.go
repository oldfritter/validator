...
func RegisterDefaultTranslations(v *validator.Validate, trans ut.Translator) (err error) {
	...
	translations := []struct {
		...
		},
		{
			...
			translation: "{0} moet een geldig porseleinsnummer zijn",
			override:    false,
		},
		{
			tag:         "chinese_mobilephone",
			translation: "{0} moet een geldig China Mobile-telefoonnummer zijn",
			override:    false,
		},
		{
			tag:         "url",
		...
		{
			tag:         "notin",
			translation: "{0} mag niet een van [{1}] zijn",
			override:    false,
			customTransFunc: func(ut ut.Translator, fe validator.FieldError) string {
				s, err := ut.T(fe.Tag(), fe.Field(), fe.Param())
				if err != nil {
